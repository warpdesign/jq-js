...
this.createWithFriesXML = function() {
	...
	return jq.parseXML( string.replace( /\{\{\s*externalHost\s*\}\}/g, externalHost ) );
};
...
this.ajaxTest = function( title, expect, options ) {
	...
	asyncTest( title, expect, function() {
		...
		var completed = false,
			...
			requests = jq.map( requestOptions, function( options ) {
				var request = ( options.create || jq.ajax )( options ),
		...
		ajaxTest.abort = function( reason ) {
			if ( !completed ) {
				...
				jq.each( requests, function( i, request ) {
					request.abort();
				});
			}
		};
...
this.testIframe = function( fileName, name, fn ) {
	...
	test(name, function() {
		...
		var iframe = loadFixture(),
			...
			interval = setInterval( function() {
				if ( win && win.jq && win.jq.isReady ) {
					clearInterval( interval );
					// continue
					start();
					// call actual tests passing the correct jQuery instance to use
					fn.call( this, win.jq, win, win.document );
	...
	function loadFixture() {
		var src = url("./data/" + fileName + ".html"),
			iframe = jq("<iframe />").appendTo("body")[0];
...
this.testIframeWithCallback = function( title, fileName, func ) {
	...
	test( title, function() {
		...
		iframe = jq( "<div/>" ).append(
			jq( "<iframe/>" ).attr( "src", url( "./data/" + fileName ) )
		).appendTo( "body" );
	});
};
